...
const makeFakeAccount = (): CreateAccountModel => (
  {
    name: 'valid_name',
...
describe('DbAddAccount UseCase', () => {
  ...
  test('Should call CreateAccountRepository with correct data ', async () => {
    ...
    await sut.create(makeFakeAccount())
    expect(createSpy).toHaveBeenCalledWith(Object.assign({}, makeFakeAccount() , { password: 'hashed_password' }))
  })
  ...
  test('Should return an account on success', async () => {
    const { sut } = makeSut()
    const account = await sut.create(makeFakeAccount())
    expect(account).toEqual(Object.assign({}, makeFakeAccount() , { password: 'hashed_password', id: 'valid_id' }))
  })
})
